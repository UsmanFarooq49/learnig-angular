...
import { BRAND_PRIMARY } from '@/app/layout/theme/brand-theme';
    ...
    template: `
        <div class="flex flex-col gap-4">
            ...
            <div class="flex flex-col gap-2">
                ...
                <p-selectbutton [options]="themeOptions" [ngModel]="isDarkTheme() ? 'dark' : 'light'" (ngModelChange)="onThemeChange($event)" [allowEmpty]="false" size="small" />
            </div>
        </div>
    `,
    host: {
...
export class AppConfigurator {
    /** When true, render as a plain inline panel (e.g. inside a modal) instead of a floating popover. */
    inline = input<boolean>(false);

    /** Popover chrome by default; a bare block when embedded inline (the modal supplies its own chrome). */
    ...
    primaryColors = computed<SurfacesType[]>(() => {
        ...
        const palettes: SurfacesType[] = [
            { name: 'noir', palette: {} },
            { name: 'brand', palette: BRAND_PRIMARY as unknown as SurfacesType['palette'] },
        ];
    ...
    onThemeChange(value: string) {
        this.layoutService.layoutConfig.update((prev) => ({ ...prev, darkTheme: value === 'dark' }));
    }
}
